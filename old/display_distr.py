...
class distribution(object):
    ...
    def plot_contourf(self,subplt,lim=None,mag=False):
        ...
        self._do_contour(vx,vy,dist2D_xy.transpose(),label[0],label[1],lim=lim)
       
        subplot(subplt[0],subplt[1],subplt[2]+1)
        self._do_contour(vx,vz,dist2D_xz.transpose(),label[0],label[2],lim=lim)
        
        subplot(subplt[0],subplt[1],subplt[2]+2)
        self._do_contour(vy,vz,dist2D_yz.transpose(),label[1],label[2],lim=lim)
    ...
    def _do_contour(self,xx,yy,dist2D,xlab,ylab,lim=None,linear=False):
        if linear==False:
        ...
        plot([-10.,10.],[0.,0.],'--',color='b')
        plot([0.,0.],[-10.,10.],'--',color='b')
        xlabel(xlab)
        ylabel(ylab)
        xlim([-lim,lim])
        ylim([-lim,lim])
...
if __name__=='__main__':
    ...
    dist_e.plot_contourf([3,3,1],lim=0.75,mag=mag)
    dist_h.plot_contourf([3,3,4],lim=0.25,mag=mag)
    dist_o.plot_contourf([3,3,7],lim=0.25,mag=mag)
    tight_layout()
    subplots_adjust(bottom=0.06,top=0.94,hspace=0.3)
    
    if 'save' in sys.argv:
        savefig(sys.argv[-1]+'i_%i_j_%i' % (i0,j0))
    else:
        show()
    close('all')
